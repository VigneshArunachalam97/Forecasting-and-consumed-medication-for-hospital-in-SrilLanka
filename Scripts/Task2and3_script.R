...
saveWorkbook(wb, "Output/Summary_drug_Wise_all_Year_and_hospitals_subset_Feb_25_24.xlsx", overwrite = TRUE)
...
saveWorkbook(period1, "Output/Summary_drug_Wise_all_Year_and_hospitals_subset_period1_Feb_25_24.xlsx", overwrite = TRUE)
...
saveWorkbook(period2, "Output/Summary_drug_Wise_all_Year_and_hospitals_subset_period2_Feb_25_24.xlsx", overwrite = TRUE)
...
names(colr) <- unique(mel$HID_new)
drug_name <- gsub("\\/", "_", drugs)
...
for(drug in 1:length(drugs)){
  ...
  plot <- ggplot(data = try, aes(x = HID_new, y = Stock,  color = Years, group = Years)) + 
    ...
    theme(legend.position = "bottom", 
          plot.title = element_blank()) 
  png(paste0("Plots/Figures_without_title_hospitals/line_plot_", drug_name[drug], ".png"),  w = 4000, h = 2500, unit = "px", res = 400)
  print(plot)
  dev.off()
}
...
for(drug in 1:length(drugs)){
  ...
  png(paste0("Plots/Figures_with_title_hospitals/line_plot_", drug_name[drug], ".png"),  w = 4000, h = 2500, unit = "px", res = 400)
  print(plot)
  dev.off()
}
...
#png("try.png", w = 4000, h = 2500, unit = "px", res = 400)
ggplot(data = try, aes(x = HID_new, y = Stock,  color = Years, group = Years)) +
  geom_point(size = 1.2) +
  geom_line(size = 1.5, alpha = 0.7) +
  scale_color_manual(values = colr_for_year, name = "Years") +
  theme_minimal() +
...
# dev.off()
# 
# pdf("stock_code_200201_bar.pdf", w = 7, h= 5)
# ggplot(data = mel, aes(x = variable, y = value, fill = HID)) + 
#   geom_bar(stat = "identity", position = "dodge", width = 0.7) +  # Grouped bar plot
